...
```bash
git checkout -- docs/ui-audit-2026-09-19/S8/screenshots/ docs/ui-audit-2026-09-19/fixes/
```

**`fixes/W*/screenshots/` 同样会被覆盖** —— 重跑 w1/w2 的 fix spec 会盖掉「修复后」
的取证图。W3-I 撞到过并还原了四个目录。
...
**提交前的一行判据**（docx track 给的，把这条教训变成可执行的检查）：

```bash
git cat-file -e HEAD:<被 import 的文件> || echo "HEAD 编译不过"
```

对你即将提交的共享文件里每个新 import 跑一遍。工作区能编译不代表 HEAD 能——
**HEAD 是一个你没有直接在上面工作过的快照**，这正是它容易被忽略的原因。
...
## 3.5 量具错误的第三例：协调方自己（记下来，因为前两例都是别人）

裁决 W3-I 与另一 session 关于「哪些色值有原型依据」的争议时，协调方判定
`#617f6f` 是 base64 假阳性。**这个判定是错的**，W3-I 独立查出原型里它是真 CSS：

```
#app #task .step.done{color:#617f6f}      （第 175,748 字节）
```

正是 `agent.css` 的 `.shell-task-step.is-done` 所移植的那条。

错在量具：用的模式是 `grep -o ".\{60\}617f6f.\{20\}"`，**要求匹配前有 60 个同行
字符**，而那一行只有 22 个字符，于是 grep 无输出；紧跟其后的
「无可读上下文 = 落在 base64 里，是假阳性」是一句**无条件 echo**，并非由 grep 结果
推出。两件事叠加，一个不成立的结论被当成了实测。

可靠的查法（W3-I 给的，已在报告 §1.4 留了脚本）：**按字节偏移取前后 160 字节看有
没有选择器**，不要用要求固定前后文长度的正则。

至此本轮量具错误三例，肇事方各不相同：
1. 只扫四个文件就当成全部（抽样不全）
2. 量恢复后的磁盘当成「未损坏」的证据（基线取错）
3. 正则要求的上下文长度超过实际行长，加一句无条件的结论 echo（**本例**）

三例的共同形状仍然是那句：**量具对着错的东西，和量具坏掉，在输出上一模一样。**
第三例还多一层：结论不是被量出来的，是被预先写好的。

## 3.6 两个会咬到后来人的测量陷阱（W3-I 发现）

- **视觉指纹按文档下标做 key 会骗人。** 共享树里另一 session 插了 11 个 DOM 节点，
  指纹报出 **886 个「moved」**，其中一个真正的变化都没有。改成从 `#shell` 起的
  `tag.class` 路径后，同样的插入报 `0 changed, 11 added`。按下标 key 的快照闸门
  第一天就会被关掉。
- **CSSOM 无法表示 `var()` 简写 + 同盒长写。** `.shell-task-spinner` 上
  `getPropertyValue("border")` 返回空串、`cssText` 直接丢掉 border，**而且只在替换
  之后才失效** —— 教科书级的假阳性。正确处理是加一个真渲染并断言四条边的 pass
  （带硬编码期望值，样式表没加载时会红而不是静默匹配），而不是为迁就测量去改样式表。

## 3.7 逐条对账：124 条里有 32 条从未被任何 fix 报告提及

前面几波的进度一直是各 track **自报关闭数的累加**（74 条）。真正逐 ID 对账的结果：

```
普查总发现                      124
被任一 fix 报告提及（含「没关掉」）  92
从未被任何 fix 报告提及            32
```
...
| 分片 | 条数 | 原因 |
|---|---|---|
| S3 | **10** | **`home/` 与 composer 的布局，Wave 1 与 Wave 2 没有任何 track 拥有它们** |
| S6 | 7 | 暗色（已决定不做）、过渡中间态、加载态、骨架行数 |
| S4 | 4 | 画布契约，被 docx track 占着文件 |
| S7 | 4 | 部分实为重复计量（S7-001 = S2-002，已由 W1-A 关掉但未按此 ID 记账） |
| S8 | 3 | 空态文案与重复死控件 |
| S1 | 2 | 折叠轨下新建文件夹不可达、editor 侧栏空白 |
| S5 | 2 | `DESIGN.md`（已由产品拍板解决）、`data-loaded` 零消费 |

**最大一块 S3 的 10 条与 MERGE-001 是同一个规划缺口**：按「最大根因」切分工时，
根因表里没有「composer / home 的行内布局」这一项，于是那片表面没有 owner。
按根因分工能最大化单次改动的收益，代价是**不在任何根因簇里的单点缺陷会集体无主**——
这是这种分工方式的固有成本，不是执行失误。

已开 W5 收这批里当前不被占用的部分；落在别人文件里的只写修法不动手。
...
21 笔已提交在 `develop/1.0`。干净工作树（HEAD，不含任何并发 session 的在制品）上：

```
tsc --noEmit          退出码 0
vitest run            189 文件 / 1349 用例全绿
12 条 fix/gate spec   130 passed，零 skip
verify:shell:styles   pass 6 / fail 0
```

逐 ID 对账：**124 条里 107 条已处理**（关掉、或写明为何不关），**17 条仍未提及**。
（S3-006 —— 首页主按钮恒为「停止任务」那条 P1 —— 已由 docx track 于 `3520be8` 修掉，两处同改。）

### 这 18 条为什么还在

| 归属 | 条数 | 情况 |
|---|---|---|
| `composer/Composer.tsx` 被并发 session 持有 | 6（S3-005/007/008/009/010/011） | 修法均已查明。其中 **S3-006 是 P1** 且已交付确切改法（`:230` 加 `placement !== "home"`，**且 `:881` 必须同步改 `disabled`**，只改一处会留下一个 enabled 却什么都不做的按钮） |
| 落在 `src/canvas/DocxCanvas.tsx`（他人在制品） | 2（S4-014/016） | 通道与常量已备好，加两行即可 |
| 重复计量 | 3（S7-001 = S2-002、S7-010 = S2-008、S6-017 = R18） | 实际已由 W1-A / W1-C / W3-J 关掉，只是没按这些 ID 记账 |
| 跨越两个 track 的边界 | 4（S6-002/004 过渡中间态、S8-012/013 重复死控件与反馈位置） | 现象已被 W1-B/W2-F 的改动部分改善，但没有人按这些 ID 验收 |
| 产品决策 / 功能缺失 | 3（S5-010 `data-loaded` 零消费已由 W5 部分解决、S7-011 i18n 欠账、S7-013 Custom 权限无 UI） | 不是样式缺陷 |

**没有一条是「查不出来」或「不知道怎么改」**——全部要么在别人手上、要么是记账口径、要么需要产品决定。

### 仍然欠着的工程债

- **遮挡检测闸门未落地**（W4b 明确说明）：它必须是运行时的（`elementFromPoint`），
  要渲染树而不是选择器文本，两条静态闸门都做不到。
- **字重闸门无法存在**：`tokens.css` 没有 weight 一节，而 17 处裸 `font-weight` 散在各文件。
