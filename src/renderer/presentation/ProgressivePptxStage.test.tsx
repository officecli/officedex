...
describe("ProgressivePptxStage", () => {
  ...
  it("shows outline intervention and starts drawing explicitly", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /确认大纲并开始绘制/i }));
  ...
  it("shows explicit processing feedback instead of a confirmation dead end", () => {
    ...
    expect(screen.getByRole("heading", { name: "正在理解制作方向" })).toBeInTheDocument();
    expect(screen.getByRole("status")).toHaveTextContent("正在整理需求并规划页面");
    expect(screen.queryByRole("button", { name: /确认方向/i })).toBeNull();
