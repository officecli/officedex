      ...
      {snapshot && sheet && step === "preview" ? (
        <div className="app-builder__preview-stage">
          <aside className="app-builder__chat">
            ...
            <div className="app-builder__chat-compose"><TextArea rows={3} value={revisionPrompt} placeholder={t("appBuilder.chat.placeholder")} onChange={(event) => setRevisionPrompt(event.target.value)} /><Button className="od-button--icon-submit" variant="primary" size="small" ariaLabel={t("appBuilder.chat.apply")} title={t("appBuilder.chat.apply")} icon={<ChevronRight />} disabled={!revisionPrompt.trim()} onClick={() => { setPrompt((current) => `${current}\n${revisionPrompt.trim()}`); setRevisionPrompt(""); }} /></div>
          </aside>
