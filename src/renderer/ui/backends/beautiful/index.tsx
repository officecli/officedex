...
export function Dialog({ open = false, title, children, onClose, className = "", ...rest }: DialogProps) { const t = useT(); if (!open) return null; return <div className="od-dialog-backdrop" role="presentation" onClick={(event) => { if (event.target === event.currentTarget) onClose?.(); }}><div {...rest} role="dialog" aria-modal="true" className={`od-dialog ${className}`.trim()}><div className="od-dialog-header"><strong>{title}</strong>{onClose ? <button type="button" aria-label={t("ui.text.Close")} className="od-dialog-close" onClick={onClose}>×</button> : null}</div><div className="od-dialog-body">{children}</div></div></div>; }
