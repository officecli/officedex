...
function PasswordInputRoot({ visibilityLabels = { show: "Show password", hide: "Hide password" }, disabled, className, ...props }: PasswordInputProps) {
  ...
  return (
    <span className={["od-password-input", className].filter(Boolean).join(" ")}>
      <Input {...props} className="od-password-input__field" disabled={disabled} type={visible ? "text" : "password"} />
      <button
        type="button"
        aria-label={visible ? visibilityLabels.hide : visibilityLabels.show}
        className="od-password-input__toggle"
        disabled={disabled}
