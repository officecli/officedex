...
export const initialShellState: ShellState = {
  ...
  openFileIds: [],
  activeFileId: null,
  presence: { placement: "docked", expanded: true, x: 0, y: 0, edge: null },
};
...
export function hydrateShellState(persisted: Partial<PersistedShellState>): ShellState {
  ...
  return {
    ...
    navWidth: clamp(persisted.navWidth ?? initialShellState.navWidth, NAV_MIN_WIDTH, NAV_MAX_WIDTH),
    navCollapsed: persisted.navCollapsed ?? false,
    taskWidth: clamp(persisted.taskWidth ?? initialShellState.taskWidth, TASK_MIN_WIDTH, TASK_MAX_WIDTH),
    ...
    presence: {
      ...
      x: Number.isFinite(presence?.x) ? Number(presence?.x) : 0,
      y: Number.isFinite(presence?.y) ? Number(presence?.y) : 0,
