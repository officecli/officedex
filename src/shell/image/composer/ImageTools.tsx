...
const MODELS: ImageModelOption[] = [
  { id: "auto", name: "Auto", mark: <Sparkles size={14} strokeWidth={1.7} aria-hidden="true" />, tone: "auto", detail: "Let OfficeDex choose for this task.", available: true },
...
const ratioLabel = (ratio: string) => (ratio === "auto" ? "Auto" : ratio === "custom" ? "Custom" : ratio);
  ...
  return (
    <div className={`shell-ig-tools${compact ? " is-compact" : ""}`} role="group" aria-label="Image options">
      ...
      <button
        ...
        aria-label="Mention files or folders"
        title="Mention files or folders"
        disabled={disabled}
        onClick={onMention}
      >
      ...
      {open === "camera" ? (
        <ImagePopover anchor={anchors.camera} title="Camera" width={520} onClose={close}>
          <div className="shell-ig-camera-switch">
            <span>Use camera settings</span>
            <button
              type="button"
              className="shell-ig-switch"
              role="switch"
              aria-label="Use camera settings"
          ...
          <p className="shell-ig-hint">Shape the camera and lens look of your image.</p>
          <div className="shell-ig-camera-grid" data-muted={String(!draft.camera.enabled)}>
            {CAMERA_FIELDS.map((field) => (
              <div key={field.key} className="shell-ig-camera-column">
                <span className="shell-ig-camera-label">{field.label}</span>
      ...
      {open === "style" ? (
        <ImagePopover anchor={anchors.style} title="Image style" width={350} onClose={close}>
          <div className="shell-ig-list" role="radiogroup" aria-label="Image styles">
  ...
  return (
    <>
      <div className="shell-ig-block">
        <h3>Aspect ratio</h3>
        <div className="shell-ig-ratios" role="radiogroup" aria-label="Image aspect ratio">
      ...
      <div className="shell-ig-block">
        <h3>Resolution</h3>
        <div className="shell-ig-segments" role="radiogroup" aria-label="Image resolution">
      ...
      <div className="shell-ig-block">
        ...
        <div className="shell-ig-segments shell-ig-count" role="radiogroup" aria-label="Number of images">
      ...
      <div className="shell-ig-block">
        ...
        <div className="shell-ig-dimensions">
          {(["width", "height"] as const).map((edge, index) => (
            <label key={edge}>
              <span>{edge === "width" ? "W" : "H"}</span>
              <input
                type="number"
                min={MIN_IMAGE_EDGE}
                max={MAX_IMAGE_EDGE}
                step={1}
                aria-label={`Image ${edge}`}
        ...
        {draft.ratio === "auto" ? <p className="shell-ig-hint">Auto lets the model pick the shape; the size applies once you choose a ratio.</p> : null}
      </div>
    </>
  );
}
  ...
  return (
    <div className="shell-ig-summary">
      {style && style.value !== "auto" ? (
        <button type="button" disabled={disabled} aria-label="Remove image style" title="Remove image style" onClick={() => onChange({ style: "auto" })}>
      ...
      {draft.camera.enabled ? (
        <button
          type="button"
          disabled={disabled}
          aria-label="Remove camera settings"
          title="Remove camera settings"
