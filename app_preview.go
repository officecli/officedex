...
// SavePptxEditorVideoInput carries an MP4 generated by the embedded
// FFmpeg.WASM encoder. Binary content travels as base64 at the Wails boundary.
type SavePptxEditorVideoInput struct {
	PreviewToken  string `json:"previewToken"`
	SessionID     string `json:"sessionId"`
	Revision      int    `json:"revision"`
...
// SavePptxEditorVideo persists an MP4 generated by the embedded presentation
// editor. The FFmpeg/WASM encoder remains in the client; Go only performs the
// revision check and atomic file write in the user's Downloads directory.
func (a *App) SavePptxEditorVideo(input SavePptxEditorVideoInput) (pptxeditor.VideoSaveResult, error) {
	if a.pptxEditorService == nil {
		return pptxeditor.VideoSaveResult{}, errPptxEditorUnavailable
	}
	data, err := base64.StdEncoding.DecodeString(input.ContentBase64)
	if err != nil {
		return pptxeditor.VideoSaveResult{}, fmt.Errorf("decode PPTX editor video: %w", err)
	}
	return a.pptxEditorService.SaveVideo(input.PreviewToken, input.SessionID, input.Revision, input.FileName, data)
}
