...
const OUTPUT_BASE = path.join(
  OFFICEDEX_ROOT,
  "build/aippt-chart-source-evidence/combo-source-reconstruction",
);
...
const FACTS = path.join(
  OFFICEDEX_ROOT,
  "skills/aippt-jssdk-design/chart/evidence/combo-source-reconstruction.json",
);
...
try {
  ...
  const report = {
    ...
    chart: {
      ...
      kind: chartKinds.length > 1 ? "combo" : (chartKinds[0] ?? null),
    },
    exactSolution: false,
  ...
  const facts = {
    id: "combo-source-reconstruction",
    family: "chart_analysis",
    source:
      "sampleall/chart/基础属性/簇状柱形图&次坐标轴上的折线图/簇状柱形图&次坐标轴上的折线图.pptx",
    page: 2,
    program: PROGRAM_RELATIVE,
    verification:
      "skills/aippt-jssdk-design/chart/evidence/combo-source-reconstruction.verification.json",
    observed_items: 3,
    mechanism: "native-combo-column-line-secondary-axis",
    source_fact:
      "白底 16:9 画布中央放置原生组合图：主轴簇状柱、次轴虚线折线，柱顶数值标签，顶部图例。",
    transfer_hypothesis:
      "保留原生 combo chartSpace、次坐标轴折线和柱顶标签；自由页面可拆成双栏分析，但不能把组合图改成两张无关的装饰图。",
