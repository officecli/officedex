...
const REFERENCE_PATH = path.join(
  REPO_ROOT,
  "sampleall/chart/基础属性/圆环圆/圆环图/幻灯片2.png",
);
const OUTPUT_BASE = path.join(
  OFFICEDEX_ROOT,
  "build/aippt-chart-source-evidence/donut-source-reconstruction",
);
...
const FACTS = path.join(
  OFFICEDEX_ROOT,
  "skills/aippt-jssdk-design/chart/evidence/donut-source-reconstruction.json",
);
...
try {
  ...
  const facts = {
    id: "donut-source-reconstruction",
    family: "chart_distribution",
    source:
      "sampleall/chart/基础属性/圆环圆/圆环图.pptx",
    page: 2,
    program: PROGRAM_RELATIVE,
    verification:
      "skills/aippt-jssdk-design/chart/evidence/donut-source-reconstruction.verification.json",
    observed_items: 4,
    mechanism: "native-donut-default-style",
    source_fact:
      "白底 16:9 画布中央放置原生圆环图，标题为销售额，无图例，扇区带数值标签。",
    transfer_hypothesis:
      "保留原生 doughnut chartSpace、75% 内孔、扇区配色和数值标签；自由页面可在外层加引线，但不能把环图改成普通列表。",
