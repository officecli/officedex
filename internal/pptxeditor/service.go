...
// SaveVideo persists an MP4 generated by the embedded JS/WASM encoder in the
// user's Downloads directory. The revision fence prevents a stale export from
// being presented as the result of newer edits.
func (s *Service) SaveVideo(previewToken, sessionID string, revision int, fileName string, data []byte) (VideoSaveResult, error) {
	if len(data) == 0 || int64(len(data)) > maxVideoBytes {
		return VideoSaveResult{}, errors.New("pptx editor: invalid video size")
	}
	cleanName, err := safeVideoFileName(fileName)
	if err != nil {
		return VideoSaveResult{}, err
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	session, err := s.sessionLocked(previewToken, sessionID)
	...
	if revision != session.revision {
		return VideoSaveResult{}, fmt.Errorf("pptx editor: revision conflict: current=%d", session.revision)
	}
	if err := validateSourceUnchanged(session); err != nil {
		return VideoSaveResult{}, err
	}
	...
	if err != nil {
		return VideoSaveResult{}, fmt.Errorf("pptx editor: resolve home directory: %w", err)
	}
	directory := filepath.Join(home, "Downloads")
	if err := os.MkdirAll(directory, 0o755); err != nil {
		return VideoSaveResult{}, fmt.Errorf("pptx editor: create Downloads directory: %w", err)
	}
	destination := uniqueVideoDestination(directory, cleanName)
	if err := atomicfile.WriteFile(destination, data, 0o644); err != nil {
		return VideoSaveResult{}, fmt.Errorf("pptx editor: save video: %w", err)
	}
	return VideoSaveResult{FilePath: destination, FileName: filepath.Base(destination)}, nil
}

func safeVideoFileName(value string) (string, error) {
	...
	if name == ".mp4" {
		return "", errors.New("pptx editor: video file name is invalid")
	}
	return name, nil
}

func uniqueVideoDestination(directory, name string) string {
	...
	for index := 2; ; index++ {
		candidate := filepath.Join(directory, fmt.Sprintf("%s-%d.mp4", base, index))
